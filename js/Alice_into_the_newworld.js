...
$(window).scroll(function(){
    ...
    if (scroll == p3) {
        ...
        let typingIdx = 0;
        let liIndex = 0;
        let liLength = $(".typing_txt3>li").length;
        let tyInt;

        // 타이핑될 텍스트를 가져온다 
        let typingTxt = $(".typing_txt3>li").eq(liIndex).text();
        ...
        function typing() {
            ...
            if (typingIdx < typingTxt.length) {
                // 타이핑될 텍스트 길이만큼 반복 
                $(".typing3 li").eq(liIndex).append(typingTxt[typingIdx]);
                // 한글자씩 이어준다. 
                ...
                typingIdx = 0;
                typingBool = false;
                typingTxt = $(".typing_txt3>li").eq(liIndex).text();
